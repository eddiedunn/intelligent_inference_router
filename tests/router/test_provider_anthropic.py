...
def test_forward_to_anthropic(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(router_main, "ANTHROPIC_BASE_URL", "http://testserver")
    monkeypatch.setattr(router_main, "EXTERNAL_ANTHROPIC_KEY", "dummy")
    ...
    with registry.get_session() as session:
        registry.upsert_model(session, "claude-3", "anthropic", "unused")
